...
速度が重要なライブラリのために、非同期ロギングをサポートしています。
...
カスタムMITライセンスを採用しています。boostやfmtlibと同様に、バイナリ配布の場合は権利表記が必要ありません。
...
- 多くのロガーでは、includeと同時にグローバルロガーが定義されます。これは一見便利ですが、複数のプロジェクトで採用された場合、衝突します。
